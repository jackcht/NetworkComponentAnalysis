...
public class Data {
	...
	public void getConnectionData(String filename) throws Exception{
		...
		for (int i = 0; i < connection.size(); i++){
			...
			//System.out.println("--"+connection.get(i)[0]+"--"+connection.get(i)[1]+"--"+connection.get(i)[2]);
		}
		...
		for (int i = 0; i < this.getTranscriptFactor().length; i++){
			for (int j = 0; j < connection.size(); j++){
				if (connection.get(j)[0].equals(this.getTranscriptFactor()[i])){
					//System.out.println("connection.get("+j+")[0]: " + connection.get(j)[0] + " == "+ "getTranscriptFactor()["+i+"]: "+ this.getTranscriptFactor()[i]);
					for (int k = 0; k < this.getGeneNames().length; k++){ 
						if (connection.get(j)[2].equals(this.getGeneNames()[k])){
							//System.out.println("---- connection.get("+j+")[2]: " + connection.get(j)[2] + " == "+ "this.getGeneNames()["+k+"]: "+ this.getGeneNames()[k] + " value=="+connection.get(j)[1]);
